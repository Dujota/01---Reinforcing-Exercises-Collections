...
hash[:response][:results].each do |views, value|
  value = 0
  views[:views] = value
  # can do one element and then call that elemnent and add the key/value pair hash[:response][:results].each do |element|
  # element[:views] = 0 end
...
def display_views(hash)
  hash[:response][:results].each do |key, value|
    puts "Web Title is: #{key[:webTitle]}"
    puts "View Count is: #{key[:views]}"
    # binding.pry
  end
end

25.times { read_article(hash) }

display_views(hash)
# display_views(read_article(hash)) # playing around wiht some of the functions
